...
\## 최종 판정 (2026-08-27 실측 · 2026-09-15 소급 기입)



※ 본 절은 2026-08-27 밤 iPhone 실측 완료 직후 작성됐으나 커밋이 유실되어

&#x20;  09-15 상태 회수 중 발견, 소급 기입함. 판정 기준 변경 없음(R8).
...
| A 빌드·배포 | Go | Vercel 공개 URL 실행 확인 |

| B-1 데스크톱 로딩 | Cond.Go | 약 4초 (기준 3/5초) — 부록 6항 목표 재설정 참조 |

| B-2 모바일 로딩 | Go | 약 4.5초 (기준 7초) |

| C 모바일 성능 | Go | S23+ 60초 조작 무결 + iPhone(WebKit) 실행·조작 확인 |

| D 한글 IME | Go | 데스크톱·S23+ 가상키보드·iPhone 전부 입력→SendMessage 반영 |

| E API 왕복 P75 | Go | 데스크톱 521ms / iPhone 500ms / S23+ 733.9ms (기준 1000ms) |

| F 수식 이미지 | Go | 3기기 전부 1128×286 원본 표시 |



종합: 조건부 Go — 하이브리드 아키텍처 채택 확정, 웹 온리 폴백 폐기.

iOS 측정 환경: iPhone / Instagram in-app(WebKit). Safari 본체 직접 측정은

베타 보강 항목. 기기 모델·iOS 버전 미기록.
